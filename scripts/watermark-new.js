// watermark-new.js — 신규 단지만 타깃 워터마크 (기존 단지 재처리 방지)
// photos-original/<단지>/ → 워터마크 2방향 합성 → public/photos/<단지>/
// 사용: node scripts/watermark-new.js "알티에로" "마린시티 자이"
...
async function main() {
  ...
  if (targets.length === 0) {
    console.error('❌ 대상 단지명을 인자로 전달하세요. 예: node scripts/watermark-new.js "알티에로" "마린시티 자이"');
    process.exit(1);
  }

  let processed = 0, failed = 0;
  for (const apt of targets) {
    const aptOrig = path.join(originalDir, apt);
    if (!fs.existsSync(aptOrig)) { console.error(`⚠️ 원본 없음: ${apt}`); continue; }
    const files = walk(aptOrig);
    console.log(`\n📸 ${apt}: ${files.length}장 워터마크 처리`);
    for (const originalPath of files) {
      try {
        const rel        = path.relative(originalDir, originalPath);
        const outputPath = path.join(photosDir, rel);
